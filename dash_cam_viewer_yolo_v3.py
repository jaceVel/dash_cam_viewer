#!/usr/bin/env python3
"""
Dash Cam Viewer YOLO v3 - Box Overlay Review + Hard Negative Marking
Extends v2: manual boxes shown in blue, YOLO detections in green on the frame.
Right-click a green detection box to mark it as a hard negative (turns red, saved to false_detections.csv).
"""
...
from PyQt5.QtWidgets import (
    ...
    QScrollArea, QGridLayout, QFrame, QListWidget, QListWidgetItem, QSizePolicy
)
...
MODELS_REGISTRY_PATH = os.path.join(PROJECTS_DIR, "models.json")
FPS_FALLBACK = 30.0
...
def project_confirmed_csv(name):
    return os.path.join(PROJECTS_DIR, name, "confirmed_detections.csv")
...
with open(csv_path, 'a', newline='') as f:
    w = csv.DictWriter(f, fieldnames=["name","latitude","longitude","source_frame",
                                       "saved_image","box_x","box_y","box_w","box_h","source"])
    if write_header:
        w.writeheader()
...
active_tracks = []   # {cx, cy, area_first, area_last, first_frame, last_frame, first_lat, first_lon, cls}
vehicles_out  = []   # one dict per confirmed oncoming vehicle
next_id = 0
total = len(points)
    ...
    for t in closed:
        ...
        if t['area_last'] >= t['area_first'] * GROW_THRESH:
            out.append({
                ...
                'box':   t['last_box'],   # [x1, y1, x2, y2] in original image pixels
            })

for idx, (lat, lon, fname) in enumerate(points):
    pct = int((idx + 1) / total * 100)
    if (idx + 1) % 25 == 0 or idx == total - 1:
        print(f"PROGRESS:{pct}:{idx+1}/{total} frames | {len(active_tracks)} active tracks | {len(vehicles_out)} counted", flush=True)
...
print(f"Tracks closed: {next_id} total | {len(vehicles_out)} passed oncoming filter (grew >{GROW_THRESH}x)", flush=True)
...
class YoloTrainWorker(QThread):
    ...
    def __init__(self, model_name, training_projects, epochs=50):
        super().__init__()
        self.model_name        = model_name
        self.training_projects = training_projects  # list of project names
        self.epochs            = epochs
    ...
    def run(self):
        try:
            ...
            sources = []
            for proj in self.training_projects:
                csv_path   = os.path.join(PROJECTS_DIR, proj, "posts.csv")
                frames_dir = project_frames_dir(proj)
                if not os.path.exists(csv_path) or not os.path.isdir(frames_dir):
                    self.log.emit(f"  Skipping '{proj}': no posts.csv or frames dir")
                    continue
                ...
                self.log.emit(f"  Using posts.csv from: {proj}"
                              + (f" ({len(false_frames)} false detections)" if false_frames else ""))

            if not sources:
                self.log.emit("No valid project sources found. Box some posts first.")
                self.finished.emit(False)
                return
            ...
            self.log.emit(f"Training model '{self.model_name}', base: {os.path.basename(base)}, epochs: {self.epochs}")
            ...
            if proc.returncode == 0:
                # Update registry trained_date
                registry = load_models_registry()
                if self.model_name in registry:
                    registry[self.model_name]['trained_date'] = datetime.now().strftime("%Y-%m-%d")
                    registry[self.model_name]['training_projects'] = self.training_projects
                    save_models_registry(registry)
                self.log.emit(f"Training complete. Model: {mwp}")
                self.finished.emit(True)
            else:
...
class YoloDetectWorker(QThread):
    ...
    def __init__(self, project_name, confidence, points_with_files, model_name=''):
        super().__init__()
    ...
    def _resolve_model_path(self):
        """Resolve model path from model_name. Returns path string or None."""
        if not self.model_name:
            return None
        # Check if it is a named model in the registry with weights
        mwp = model_weights_path(self.model_name)
        if os.path.exists(mwp):
            return mwp
        # Check if model_name is a raw .pt file (pretrained)
        registry = load_models_registry()
        cfg = registry.get(self.model_name, {})
        if cfg.get('type') == 'pretrained':
            mf = cfg.get('model_file', self.model_name)
            local = os.path.join(os.path.dirname(os.path.abspath(__file__)), mf)
            if os.path.exists(local):
                return local
            return mf  # let ultralytics resolve
        return None

    def run(self):
        model_path = self._resolve_model_path()
        if not model_path:
            self.log.emit("No model found. Select a model with trained weights.")
            self.finished.emit(False)
            return
        ...
        proc = subprocess.Popen(
            [sys.executable, '-c', _DETECT_SCRIPT,
             frames_dir, model_path, str(self.confidence),
...
class VehicleCountWorker(QThread):
    ...
    def run(self):
        ...
        # Ultralytics auto-downloads known model names if not present locally
        if not os.path.exists(base_model):
            self.log.emit(f"{self.model_name} not found locally — Ultralytics will download it automatically.")
            base_model = self.model_name  # let ultralytics resolve by name
...
class BoxFrameLabel(QLabel):
    ...
    Manual boxes (drawn by user) are shown in blue.
    YOLO detection boxes are shown in green; right-click to mark as hard negative (turns red).
    """

    box_drawn = pyqtSignal(int, int, int, int)            # x1, y1, x2, y2 in original image coords
    hard_negative_marked = pyqtSignal(int, int, int, int) # x1, y1, x2, y2 of the marked detection box

    def __init__(self, parent=None):
        super().__init__(parent)
        self._boxes = []         # manual drawn boxes: list of (x1,y1,x2,y2) in orig coords
        self._detect_boxes = []  # yolo boxes: list of {'rect':(x1,y1,x2,y2), 'neg':False}
    ...
    def set_detection_boxes(self, boxes):
        """Set YOLO detection boxes. boxes = list of (x1,y1,x2,y2) in orig coords."""
        self._detect_boxes = [{'rect': b, 'neg': False} for b in boxes]
        self.update()
    ...
    def _orig_rect_to_widget(self, x1, y1, x2, y2):
        """Convert original image rect to widget QRect."""
        if not self._orig_size:
    ...
    def paintEvent(self, event):
        ...
        # Detection boxes — green (unreviewed) or red (hard negative)
...
class ViewPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.map_container.setMinimumWidth(420)
        ...
        self.image_label.hard_negative_marked.connect(self._on_hard_negative_marked)
        ...
        id_row.addWidget(QLabel("Model:"))
        self.model_combo = QComboBox()
        self.model_combo.setMinimumWidth(160)
        self.model_combo.setToolTip("Select a model from the Models registry for detection")
        id_row.addWidget(self.model_combo)
        ...
        self.remove_culvert_btn = QPushButton("Rem")
        self.remove_culvert_btn.setFixedWidth(44)
        self.remove_culvert_btn.setToolTip("Remove current culvert")
        self.remove_culvert_btn.clicked.connect(self._remove_culvert)
        culvert_row.addWidget(self.remove_culvert_btn)

        self.remove_all_culverts_btn = QPushButton("Rem All")
        self.remove_all_culverts_btn.setFixedWidth(62)
        self.remove_all_culverts_btn.setToolTip("Remove all culverts")
        ...
        self.vehicle_model_combo.addItems(_vehicle_models)
        self.vehicle_model_combo.setCurrentText('yolov8n.pt')
        self.vehicle_model_combo.setToolTip(
            "YOLO model weights for vehicle detection.\n"
            "n=nano (fastest), s=small, m=medium, l=large, x=extra-large.\n"
            "yolo11 series is newer and more accurate than yolov8.\n"
    ...
    def _find_culverts(self):
        ...
        if not model_name:
            QMessageBox.warning(self, "No Model", "Select a model from the Models tab first.")
            return
        # Check model has weights or is pretrained
        registry = load_models_registry()
        cfg = registry.get(model_name, {})
        if cfg.get('type') == 'custom' and not os.path.exists(model_weights_path(model_name)):
            QMessageBox.warning(self, "No Model Weights",
                                f"Model '{model_name}' has no trained weights yet. Train it in the Models tab.")
            return
    ...
    def _on_vehicles_counted(self, result):
        # result is a list of {"lat", "lon", "cls", "frame", "box"}
        ...
        # Persist to CSV so the review panel can load / remove entries
        ...
        CLASS_COLOR = {'car': 'blue', 'truck': 'red', 'bus': 'orange', 'motorcycle': 'green', 'vehicle': 'purple'}
    ...
    def _save_vehicle_report(self):
        # Prefer the CSV (which reflects any review-panel removals) over in-memory list
        vehicles = []
        if self._project_name:
            csv_path = project_vehicles_csv(self._project_name)
            if os.path.exists(csv_path):
        ...
        try:
            ...
            CLASS_COLOR = {'car': 'steelblue', 'truck': 'crimson', 'bus': 'darkorange',
                           'motorcycle': 'seagreen', 'vehicle': 'purple'}

            points = self.communicator.points_with_files  # [(lat, lon, fname), ...]
            counts = {}
            for v in vehicles:
                cls = v.get('cls', 'vehicle')
                counts[cls] = counts.get(cls, 0) + 1
            ...
            total_km = sum(haversine(points[i-1][0], points[i-1][1], points[i][0], points[i][1])
                           for i in range(1, len(points)))
            ...
            with PdfPages(path) as pdf:
                ...
                # ── Page 2: Route map with markers ───────────────────────────
                fig, ax = plt.subplots(figsize=(11.69, 8.27))

                lats = [p[0] for p in points]
                lons = [p[1] for p in points]

                # Set extent with padding before adding basemap
                ...
                # Try satellite basemap (contextily), fall back to OSM, then plain grid
                try:
                ...
                ax.plot(lons, lats, '-', color='white', linewidth=1.5,
                        alpha=0.6, label='Route', zorder=2)
                # Start/end markers
                ...
                # Detection table (up to 50 rows)
                ax_tbl.axis('off')
                display = vehicles[:50]
                col_labels = ['#', 'Class', 'Latitude', 'Longitude']
                rows = [[str(i+1), v.get('cls',''), f"{v['lat']:.5f}",
                         f"{v['lon']:.5f}"]
                ...
                for page_start in range(0, len(vehicles), per_page):
                    ...
                    for ax, v in zip(axes, batch):
                        ...
                        color = CLASS_COLOR.get(cls, 'purple')
                        ax.set_title(
                            f"{cls.capitalize()}  {v['lat']:.5f}, {v['lon']:.5f}",
    ...
    def load_models_combo(self):
        """Populate the model combo from the models registry."""
        current = self.model_combo.currentText()
        self.model_combo.blockSignals(True)
        self.model_combo.clear()
        registry = load_models_registry()
        for mname in sorted(registry.keys()):
    ...
    def _on_detect_finished(self, success):
        ...
        # Refresh boxes for current frame now that detection results are in posts.csv
        if self.communicator and self.communicator.current_index is not None:
            self._reload_frame_boxes(self.communicator.current_index)
    ...
    def _on_frame_changed(self, index):
        """Clear drawn boxes when navigating to a new frame and reload from posts.csv."""
        self._detected_boxes = []
        self.image_label.clear_boxes()
        if self.box_chk.isChecked():
            self.image_label.set_draw_mode(True)
        self._reload_frame_boxes(index)

    def _reload_frame_boxes(self, index):
        """Load manual and YOLO boxes for the current frame from posts.csv and display them."""
        if not self._project_name or not self.communicator:
            return
        if index is None or index >= len(self.communicator.points_with_files):
            return
        _, _, fname = self.communicator.points_with_files[index]
        csv_path = project_posts_csv(self._project_name)
        if not os.path.exists(csv_path):
            return
        manual_boxes = []
        ...
        try:
            with open(csv_path, newline='') as f:
                for row in csv.DictReader(f):
                    ...
                    if src == 'yolo':
                        detect_boxes.append(rect)
                    else:
                        manual_boxes.append(rect)
        except Exception:
            return
        # Restore manual boxes into the label's internal list (no signal emitted)
        self.image_label._boxes = manual_boxes
        self.image_label.set_detection_boxes(detect_boxes)
        ...
        with open(csv_path, "a", newline="") as f:
            ...
            if write_header:
                w.writerow(["name", "latitude", "longitude", "source_frame", "saved_image",
                             "box_x", "box_y", "box_w", "box_h", "source"])
            w.writerow([name, lat, lon, fname, img_filename,
                        x1, y1, x2 - x1, y2 - y1, "manual"])

        n = len(self.image_label._boxes)
        self.coords_label.setText(f"Saved box {n} for {fname}  ({x2-x1}x{y2-y1}px)  — posts/")

    def _on_hard_negative_marked(self, x1, y1, x2, y2):
        """Called when user right-clicks a YOLO detection box to mark it as a hard negative."""
        if not self.communicator or self.communicator.current_index is None:
            return
        if not self._project_name:
            return
        ...
        if fname not in existing_fd:
            ...
            with open(fd_csv, 'a', newline='') as f:
                ...
                if write_header:
                    w.writerow(['source_frame'])
                w.writerow([fname])
        self.coords_label.setText(f"Hard negative marked: {fname}  (box {x2-x1}x{y2-y1}px)")
...
class VehicleThumb(QLabel):
    """Clickable thumbnail showing a vehicle detection frame with bounding box."""
    toggled = pyqtSignal(str, bool)   # frame filename, is_selected

    def __init__(self, frames_dir, row, thumb_size=600):
        super().__init__()
        ...
        if not px.isNull():
            try:
                box_x1 = float(row.get('box_x1', 0))
                box_y1 = float(row.get('box_y1', 0))
                box_x2 = float(row.get('box_x2', 0))
                box_y2 = float(row.get('box_y2', 0))
                if box_x2 > box_x1 and box_y2 > box_y1:
                    # Scale box coords to thumbnail size
                    scale_x = thumb_size / px.width()
                    scale_y = thumb_size / px.height()
                    px_draw = px.scaled(thumb_size, thumb_size,
                                        Qt.KeepAspectRatio, Qt.SmoothTransformation)
                    # Recompute scale after aspect-ratio scaling
                    sx = px_draw.width()  / px.width()
                    sy = px_draw.height() / px.height()
                    painter = QPainter(px_draw)
                    cls = row.get('cls', 'vehicle')
                    color = QColor(CLASS_COLOR_HEX.get(cls, '#800080'))
                    pen = QPen(color, max(2, int(thumb_size / 75)))
                    painter.setPen(pen)
                    painter.drawRect(
                        int(box_x1 * sx), int(box_y1 * sy),
                        int((box_x2 - box_x1) * sx), int((box_y2 - box_y1) * sy),
                    )
                    painter.end()
                    self.setPixmap(px_draw)
                else:
                    self.setPixmap(px.scaled(thumb_size, thumb_size,
                                             Qt.KeepAspectRatio, Qt.SmoothTransformation))
            except Exception:
                self.setPixmap(px.scaled(thumb_size, thumb_size,
                                         Qt.KeepAspectRatio, Qt.SmoothTransformation))
        ...
        # Labels under image
        self._lbl_cls = QLabel(cls.capitalize(), self)
        self._lbl_cls.setAlignment(Qt.AlignCenter)
        color_hex = CLASS_COLOR_HEX.get(cls, '#800080')
        self._lbl_cls.setStyleSheet(f"font-size: 9px; color: {color_hex}; font-weight: bold;")
        self._lbl_cls.setGeometry(0, thumb_size + 4, thumb_size + 10, 14)

        short = self.frame_name[-28:] if len(self.frame_name) > 28 else self.frame_name
        self._lbl_frame = QLabel(short, self)
        self._lbl_frame.setAlignment(Qt.AlignCenter)
        self._lbl_frame.setStyleSheet("font-size: 8px; color: #aaa;")
        self._lbl_frame.setGeometry(0, thumb_size + 18, thumb_size + 10, 14)
...
class VehicleReviewPanel(QWidget):
    """Tab 4 — review vehicle detections, remove false positives before PDF export."""
    ...
    def _refresh(self):
        ...
        if not os.path.exists(csv_path):
            self.count_lbl.setText("No vehicles.csv found — run Count Vehicles first.")
            return
        ...
        self.count_lbl.setText(f"{len(rows)} detections")
    ...
    def _on_thumb_toggled(self, frame_name, selected):
        ...
        self.count_lbl.setText(
            f"{total} detections  |  {n} selected" if n else f"{total} detections"
        )
    ...
    def _remove_selected(self):
        ...
        reply = QMessageBox.question(
            ...
            "(This affects the PDF report but does not delete frame images.)",
...
class ReviewPanel(QWidget):
    def __init__(self):
        ...
        self._thumbs = []
        self._rows_by_img = {}   # saved_image -> row dict
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        self.refresh_btn.clicked.connect(self._refresh)
        top.addWidget(self.refresh_btn)
        self.confirm_btn = QPushButton("Confirm Selected (True +)")
        self.confirm_btn.setStyleSheet("color: white; background: #336633;")
        self.confirm_btn.clicked.connect(self._confirm_selected)
        top.addWidget(self.confirm_btn)
        self.remove_btn = QPushButton("Remove (False +)")
        self.remove_btn.setStyleSheet("color: white; background: #993333;")
        self.remove_btn.clicked.connect(self._remove_selected)
        top.addWidget(self.remove_btn)
        self.count_lbl = QLabel("")
    ...
    def _refresh(self):
        ...
        COLS = 12
        for i, row in enumerate(rows):
            img_file = row.get('saved_image', '')
            img_path = os.path.join(posts_dir, img_file)
            self._rows_by_img[img_file] = row
    ...
    def _confirm_selected(self):
        """Write selected detections to confirmed_detections.csv (true positives)."""
        if not self._selected:
            return
        if not self._project_name:
            return
        confirmed_csv = project_confirmed_csv(self._project_name)
        existing_confirmed = set()
        if os.path.exists(confirmed_csv):
            with open(confirmed_csv, newline='') as f:
                existing_confirmed = {r.get('saved_image', '') for r in csv.DictReader(f)}
        new_confirmed = []
        for img_file in self._selected:
            if img_file in existing_confirmed:
                continue
            row = self._rows_by_img.get(img_file)
            if row:
                new_confirmed.append({
                    ...
                    'box_w':        row.get('box_w', ''),
                    'box_h':        row.get('box_h', ''),
                    'saved_image':  img_file,
                })
        if new_confirmed:
            write_header = not os.path.exists(confirmed_csv)
            with open(confirmed_csv, 'a', newline='') as f:
                w = csv.DictWriter(f, fieldnames=['source_frame', 'box_x', 'box_y', 'box_w', 'box_h', 'saved_image'])
                if write_header:
                    w.writeheader()
                w.writerows(new_confirmed)
        # Deselect all
        for thumb in self._thumbs:
            if thumb._selected:
                ...
                thumb._apply_style()
        self._selected.clear()
        self.count_lbl.setText(f"{len(self._thumbs)} images  |  {len(new_confirmed)} confirmed")
    ...
    def _remove_selected(self):
        ...
        reply = QMessageBox.question(
            ...
            "These will be logged as false detections.",
            QMessageBox.Yes | QMessageBox.No
        )
        if reply != QMessageBox.Yes:
            return
        ...
        # Delete image files and collect removed rows
        removed_rows = []
        for img_file in self._selected:
        ...
        # Write to false_detections.csv
        fd_csv = project_false_detections_csv(self._project_name)
        existing_fd = set()
        if os.path.exists(fd_csv):
            with open(fd_csv, newline='') as f:
                existing_fd = {r['source_frame'] for r in csv.DictReader(f)}
        new_false = {r['source_frame'] for r in removed_rows} - existing_fd
        if new_false:
            write_header = not os.path.exists(fd_csv)
            with open(fd_csv, 'a', newline='') as f:
                w = csv.writer(f)
...
class ModelsPanel(QWidget):
    """Tab 5 — manage named models (custom trained and pretrained)."""

    def __init__(self, get_projects_fn):
        super().__init__()
    ...
    def _build_ui(self):
        ...
        left.setStyleSheet("background: #2e2e2e; border-right: 1px solid #444;")
        ...
        self.model_list.setStyleSheet(
            ...
            "QListWidget::item:selected { background: #4a6fa5; color: white; }"
        )
        self.model_list.currentTextChanged.connect(self._on_list_selection_changed)
        left_layout.addWidget(self.model_list, stretch=1)

        btn_row = QHBoxLayout()
        self.new_model_btn = QPushButton("New Model")
        self.new_model_btn.clicked.connect(self._new_model)
        self.delete_model_btn = QPushButton("Delete")
        self.delete_model_btn.clicked.connect(self._delete_model)
        btn_row.addWidget(self.new_model_btn)
        btn_row.addWidget(self.delete_model_btn)
        left_layout.addLayout(btn_row)
        root.addWidget(left)

        # ── Right panel: stacked (placeholder / detail form) ─────────────────
        right = QWidget()
    ...
    def _build_detail_form(self):
        dl = self._detail_layout

        # ── Type selector ──
        type_row = QHBoxLayout()
        type_row.addWidget(QLabel("Type:"))
        self.type_combo = QComboBox()
        self.type_combo.addItems(["custom", "pretrained"])
        ...
        self.name_edit.setPlaceholderText("e.g. my_posts")
        name_row.addWidget(self.name_edit)
        dl.addLayout(name_row)

        # ── Custom fields (class label, base model, training projects, epochs) ──
        self.custom_group = QGroupBox("Custom Model Settings")
        cg_form = QFormLayout(self.custom_group)

        self.class_label_edit = QLineEdit()
        self.class_label_edit.setPlaceholderText("e.g. post")
        cg_form.addRow("Class Label:", self.class_label_edit)

        self.epochs_spin = QSpinBox()
        self.epochs_spin.setRange(1, 500)
        self.epochs_spin.setValue(50)
        cg_form.addRow("Epochs:", self.epochs_spin)

        # Training projects — checkboxes
        proj_lbl = QLabel("Training Projects:")
        cg_form.addRow(proj_lbl)
        self.proj_check_widget = QWidget()
        self.proj_check_layout = QVBoxLayout(self.proj_check_widget)
        self.proj_check_layout.setContentsMargins(0, 0, 0, 0)
        self.proj_check_layout.setSpacing(2)
        self._proj_checkboxes = {}
        cg_form.addRow(self.proj_check_widget)

        dl.addWidget(self.custom_group)

        # ── Pretrained fields (model file, COCO classes) ──
        self.pretrained_group = QGroupBox("Pretrained Model Settings")
        pg_form = QFormLayout(self.pretrained_group)

        mf_row = QHBoxLayout()
        self.model_file_edit = QLineEdit()
        self.model_file_edit.setPlaceholderText("e.g. yolov8n.pt")
        mf_browse = QPushButton("Browse")
        mf_browse.clicked.connect(self._browse_model_file)
        mf_row.addWidget(self.model_file_edit)
        mf_row.addWidget(mf_browse)
        pg_form.addRow("Model File:", mf_row)

        coco_lbl = QLabel("COCO Classes:")
        pg_form.addRow(coco_lbl)
        _coco_options = [
            (0, "person"), (1, "bicycle"), (2, "car"),
            (3, "motorcycle"), (5, "bus"), (7, "truck"),
        ]
        self._coco_checkboxes = {}
        coco_widget = QWidget()
        coco_layout = QVBoxLayout(coco_widget)
        coco_layout.setContentsMargins(0, 0, 0, 0)
        coco_layout.setSpacing(2)
        for cls_id, cls_name in _coco_options:
            cb = QCheckBox(f"{cls_name} ({cls_id})")
            if cls_id in (2, 3, 5, 7):
                cb.setChecked(True)
            self._coco_checkboxes[cls_id] = cb
            coco_layout.addWidget(cb)
        pg_form.addRow(coco_widget)
        dl.addWidget(self.pretrained_group)
        ...
        self.train_btn.setStyleSheet("background: #336699; color: white;")
        ...
        dl.addWidget(self.status_lbl)

        # ── Log output ──
        log_lbl = QLabel("Log:")
        dl.addWidget(log_lbl)
        self.log_edit = QTextEdit()
        self.log_edit.setReadOnly(True)
        self.log_edit.setFont(QFont("Courier New", 9))
        self.log_edit.setMaximumHeight(160)
        dl.addWidget(self.log_edit)

        # ── Evaluate section ──
        eval_group = QGroupBox("Evaluate Model")
        eval_layout = QHBoxLayout(eval_group)
        eval_layout.addWidget(QLabel("Project:"))
        self.eval_combo = QComboBox()
        self.eval_combo.setMinimumWidth(160)
        eval_layout.addWidget(self.eval_combo)
        self.eval_btn = QPushButton("Run Evaluate")
        self.eval_btn.clicked.connect(self._evaluate)
        eval_layout.addWidget(self.eval_btn)
        self.eval_result_lbl = QLabel("")
        self.eval_result_lbl.setWordWrap(True)
        eval_layout.addWidget(self.eval_result_lbl, stretch=1)
        dl.addWidget(eval_group)
    ...
    def _on_type_changed(self, mtype):
        is_custom = (mtype == "custom")
        self.custom_group.setVisible(is_custom)
        self.pretrained_group.setVisible(not is_custom)
        self.train_btn.setVisible(is_custom)

    def refresh_models(self):
        """Reload model list from registry."""
        self.model_list.blockSignals(True)
        current = self.model_list.currentItem()
        current_text = current.text() if current else None
        self.model_list.clear()
        registry = load_models_registry()
        for mname, cfg in sorted(registry.items()):
            mtype = cfg.get('type', 'custom')
            trained = cfg.get('trained_date', '')
            if mtype == 'custom':
                suffix = f"  [{trained}]" if trained else "  [untrained]"
            else:
                suffix = "  [pretrained]"
            item_text = f"{mname}{suffix}"
            self.model_list.addItem(item_text)
            # Store actual model name as item data via UserRole
            item = self.model_list.item(self.model_list.count() - 1)
            item.setData(Qt.UserRole, mname)
        # Restore selection
        if current_text:
            for i in range(self.model_list.count()):
                if self.model_list.item(i).data(Qt.UserRole) == current_text:
                    self.model_list.setCurrentRow(i)
                    break
        self.model_list.blockSignals(False)

    def refresh_projects(self):
        """Update project checkboxes and evaluate combo."""
        projects = self._get_projects()
        # Update checkboxes
        checked = {p for p, cb in self._proj_checkboxes.items() if cb.isChecked()}
        for i in reversed(range(self.proj_check_layout.count())):
            item = self.proj_check_layout.takeAt(i)
            if item.widget():
                item.widget().deleteLater()
        self._proj_checkboxes.clear()
        for p in projects:
            cb = QCheckBox(p)
            cb.setChecked(p in checked)
            self._proj_checkboxes[p] = cb
            self.proj_check_layout.addWidget(cb)
        # Update evaluate combo
        self.eval_combo.blockSignals(True)
        cur_eval = self.eval_combo.currentText()
        self.eval_combo.clear()
        for p in projects:
            self.eval_combo.addItem(p)
        idx = self.eval_combo.findText(cur_eval)
        if idx >= 0:
            self.eval_combo.setCurrentIndex(idx)
        self.eval_combo.blockSignals(False)

    def _on_list_selection_changed(self, text):
        # Find model name from UserRole
        for i in range(self.model_list.count()):
            item = self.model_list.item(i)
            if item and item.text() == text:
                mname = item.data(Qt.UserRole)
                if mname:
                    self._on_model_selected(mname)
                    return
    ...
    def _on_model_selected(self, model_name):
        ...
        self._on_type_changed(mtype)

        if mtype == 'custom':
            self.class_label_edit.setText(cfg.get('class_label', ''))
            self.epochs_spin.setValue(cfg.get('epochs', 50))
            trained_projects = cfg.get('training_projects', [])
            for p, cb in self._proj_checkboxes.items():
                cb.setChecked(p in trained_projects)
            trained_date = cfg.get('trained_date', '')
            weights_exist = os.path.exists(model_weights_path(model_name))
            if trained_date and weights_exist:
                self.status_lbl.setText(f"Trained: {trained_date}  |  Weights: {model_weights_path(model_name)}")
            elif weights_exist:
                self.status_lbl.setText(f"Weights found (no trained_date recorded)")
            else:
                self.status_lbl.setText("Not yet trained")
        else:
            self.model_file_edit.setText(cfg.get('model_file', ''))
            coco_classes = cfg.get('coco_classes', [2, 3, 5, 7])
            for cls_id, cb in self._coco_checkboxes.items():
                cb.setChecked(cls_id in coco_classes)
            self.status_lbl.setText(f"Pretrained: {cfg.get('model_file', '')}")
    ...
    def _new_model(self):
        ...
        registry = load_models_registry()
        if name in registry:
            QMessageBox.warning(self, "Exists", f"Model '{name}' already exists.")
            return
        registry[name] = {
            "type": "custom",
            "class_label": "post",
    ...
    def _delete_model(self):
        if not self._selected_model:
            QMessageBox.information(self, "No Model", "Select a model to delete.")
            return
        reply = QMessageBox.question(
            self, "Delete Model",
            f"Delete model '{self._selected_model}'?\nThis will also remove its weights directory.",
            QMessageBox.Yes | QMessageBox.No
        )
        if reply != QMessageBox.Yes:
            return
        registry = load_models_registry()
        registry.pop(self._selected_model, None)
        save_models_registry(registry)
        # Delete weights dir
        mdir = os.path.join(MODELS_DIR, self._selected_model)
    ...
    def _browse_model_file(self):
        f, _ = QFileDialog.getOpenFileName(
            self, "Select Model Weights", filter="Model files (*.pt);;All Files (*)"
        )
        if f:
            self.model_file_edit.setText(f)

    def _save_config(self):
        name = self.name_edit.text().strip()
        if not name:
            QMessageBox.warning(self, "No Name", "Enter a model name.")
            return
        registry = load_models_registry()
        ...
        if mtype == 'custom':
            ...
            cfg['type'] = 'custom'
            cfg['class_label'] = self.class_label_edit.text().strip()
            cfg['epochs'] = self.epochs_spin.value()
            cfg['training_projects'] = [p for p, cb in self._proj_checkboxes.items() if cb.isChecked()]
            registry[name] = cfg
        else:
            cfg = registry.get(name, {})
            cfg['type'] = 'pretrained'
            cfg['model_file'] = self.model_file_edit.text().strip()
            cfg['coco_classes'] = [cls_id for cls_id, cb in self._coco_checkboxes.items() if cb.isChecked()]
            registry[name] = cfg
        save_models_registry(registry)
    ...
    def _train(self):
        ...
        if not name:
            QMessageBox.warning(self, "No Name", "Enter a model name.")
            return
        training_projects = [p for p, cb in self._proj_checkboxes.items() if cb.isChecked()]
        if not training_projects:
            QMessageBox.warning(self, "No Projects", "Select at least one training project.")
            return
        ...
        cfg['class_label'] = self.class_label_edit.text().strip()
        cfg['epochs'] = epochs
        cfg['training_projects'] = training_projects
        registry[name] = cfg
        save_models_registry(registry)
        self.log_edit.clear()
        self.train_btn.setEnabled(False)
        self.save_config_btn.setEnabled(False)
        self._train_worker = YoloTrainWorker(name, training_projects, epochs=epochs)
    ...
    def _on_train_finished(self, success):
        ...
        if success:
            self.refresh_models()
            mwp = model_weights_path(self._selected_model or '')
            self.status_lbl.setText(f"Training complete. Weights: {mwp}")
        else:
            self.status_lbl.setText("Training failed — check log above.")

    def _evaluate(self):
        if not self._selected_model:
            QMessageBox.warning(self, "No Model", "Select a model first.")
            return
        project = self.eval_combo.currentText()
        if not project:
            QMessageBox.warning(self, "No Project", "Select a project to evaluate.")
            return
        # Load confirmed detections (ground truth positives)
        confirmed_csv = project_confirmed_csv(project)
        confirmed_frames = set()
        if os.path.exists(confirmed_csv):
            with open(confirmed_csv, newline='') as f:
                for r in csv.DictReader(f):
                    confirmed_frames.add(r.get('source_frame', ''))
        total_confirmed = len(confirmed_frames)
        # Load posts.csv (detections)
        posts_csv = project_posts_csv(project)
        detected_frames = set()
        if os.path.exists(posts_csv):
            with open(posts_csv, newline='') as f:
                for r in csv.DictReader(f):
                    detected_frames.add(r.get('source_frame', ''))
        total_detected = len(detected_frames)
        # Load false detections
        fd_csv = project_false_detections_csv(project)
        false_frames = set()
        if os.path.exists(fd_csv):
            with open(fd_csv, newline='') as f:
                for r in csv.DictReader(f):
                    false_frames.add(r.get('source_frame', ''))
        tp = len(confirmed_frames & detected_frames)
        fp = len(detected_frames & false_frames)
        precision = (tp / total_detected * 100) if total_detected > 0 else 0.0
        self.eval_result_lbl.setText(
            f"Detections: {total_detected}  |  Confirmed TP: {tp}  |  "
            f"Known FP: {fp}  |  Confirmed total: {total_confirmed}  |  "
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.vehicles_btn = QPushButton("Vehicles")
        self.models_btn = QPushButton("Models")
        for btn in [self.process_btn, self.view_btn_side, self.review_btn, self.vehicles_btn, self.models_btn]:
        ...
        self.models_panel = ModelsPanel(list_projects)
        ...
        self.stack.addWidget(self.vehicle_review_panel)  # index 3
        self.stack.addWidget(self.models_panel)          # index 4
        ...
        self.vehicles_btn.clicked.connect(self._switch_to_vehicle_review)
        self.models_btn.clicked.connect(self._switch_to_models)
    ...
    def _switch_panel(self, index):
        ...
        self.vehicles_btn.setChecked(index == 3)
        self.models_btn.setChecked(index == 4)
    ...
    def _switch_to_vehicle_review(self):
        if self.current_project:
            self.vehicle_review_panel.load_project(self.current_project["name"])
        self._switch_panel(3)

    def _switch_to_models(self):
        self.models_panel.refresh_projects()
        self.models_panel.refresh_models()
