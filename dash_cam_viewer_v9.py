#!/usr/bin/env python3
"""
Dash Cam Viewer v9 - Combined Frame Extractor and Map Viewer
Culvert detection using OpenCV whitish vertical rectangle detection in box/poly area.
Box or polygon selection required for acquire. No YOLO.
Multiprocessing detection: all frames submitted to ProcessPoolExecutor in parallel.
"Show Box" renamed to "Show Selected Area" (covers both box and polygon display).
"""
...
# ─── Multiprocessing detection worker (must be top-level for pickling) ────────

def _detect_frame_job(args):
    """Run culvert detection on a single frame. Returns result dict or None."""
    fpath, norm_box, norm_poly, brightness, culverts_dir = args
    import cv2
    import numpy as np
    ...
    if norm_poly:
        pts = np.array([[int(nx * iw), int(ny * ih)] for nx, ny in norm_poly], dtype=np.int32)
        poly_mask = np.zeros((ih, iw), dtype=np.uint8)
        cv2.fillPoly(poly_mask, [pts], 255)
        masked_img = cv2.bitwise_and(img, img, mask=poly_mask)
        px, py, pw, ph = cv2.boundingRect(pts)
        px = max(0, px); py = max(0, py)
        pw = min(pw, iw - px); ph = min(ph, ih - py)
        crop = masked_img[py:py+ph, px:px+pw]
        bx, by, bw, bh = px, py, pw, ph
    else:
        nx = norm_box['x']; ny = norm_box['y']
        nw = norm_box['w']; nh = norm_box['h']
        bx = max(0, min(int(nx * iw), iw - 1))
        by = max(0, min(int(ny * ih), ih - 1))
        bw = max(1, min(int(nw * iw), iw - bx))
        bh = max(1, min(int(nh * ih), ih - by))
    ...
    mask = cv2.inRange(hsv, (0, 0, brightness), (180, 80, 255))
    ...
    for i in range(len(rects)):
        for j in range(i + 1, len(rects)):
            ax, ay, aw, ah = rects[i]
            bx2, by2, bw2, bh2 = rects[j]
            if ax > bx2:
                ax, ay, aw, ah, bx2, by2, bw2, bh2 = bx2, by2, bw2, bh2, ax, ay, aw, ah
            h_gap = bx2 - (ax + aw)
            if h_gap < 3:
                continue
            if h_gap > bw * 0.5:
                continue
            y_overlap = min(ay + ah, by2 + bh2) - max(ay, by2)
            min_h = min(ah, bh2)
            if y_overlap < min_h * 0.4:
                continue
            valid_pair = (rects[i], rects[j])
            break
        if valid_pair:
            break
    ...
    img_fn    = f"culvert_det_{ts}.jpg"
    annotated = crop.copy()
    for rx, ry, rw, rh in valid_pair:
        cv2.rectangle(annotated, (rx, ry), (rx + rw, ry + rh), (0, 255, 0), 2)
        cv2.putText(annotated, f"{rw}x{rh} b{brightness}", (rx, ry - 4),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1)
    cv2.imwrite(os.path.join(culverts_dir, img_fn), annotated,
                [int(cv2.IMWRITE_JPEG_QUALITY), 90])

    return {'img_fn': img_fn, 'bx': bx, 'by': by, 'bw': bw, 'bh': bh}
...
class SelectableFrameLabel(QLabel):
    """QLabel that supports rubber-band box selection, polygon selection, and persistent overlays."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self._rb = QRubberBand(QRubberBand.Rectangle, self)
        self._origin = QPoint()
        self._box = QRect()         # widget coords
        self._norm_box = None       # {x,y,w,h} normalised 0-1
        self._orig_size = None      # (orig_w, orig_h)
        self._selecting = False
        self._sel_enabled = False
        self.show_box = True
        # polygon
        self._poly_pts_w = []       # list of QPoint in widget coords (in-progress + final)
        self._norm_poly = None      # list of [nx,ny] normalised, or None
        self._poly_mode = False     # True while user is placing vertices
    ...
    def set_orig_size(self, orig_w, orig_h):
        ...
        if self._norm_box:
            self._recompute_box()
        if self._norm_poly:
            self._recompute_poly()

    def set_norm_box(self, norm):
        self._norm_box = norm
        self._norm_poly = None
        self._poly_pts_w = []
        if self._orig_size:
            self._recompute_box()
    ...
    def enable_poly_selection(self):
        """Start polygon placement mode. Left-click adds vertices, right-click closes."""
    ...
    def get_norm_poly(self):
        """Return list of [nx,ny] pairs or None if fewer than 3 points."""
        if not self._norm_poly or len(self._norm_poly) < 3:
            return None
        return self._norm_poly

    def set_norm_poly(self, pts):
        """Restore polygon from saved [[nx,ny],...] list."""
        self._norm_poly = pts
        self._norm_box = None
        self._box = QRect()
        if self._orig_size:
    ...
    def _recompute_poly(self):
        if not self._norm_poly or not self._orig_size:
            return
        orig_w, orig_h = self._orig_size
        lw, lh = self.width(), self.height()
        if lw <= 0 or lh <= 0:
        ...
        ox, oy = (lw - sw) // 2, (lh - sh) // 2
        self._poly_pts_w = [
            QPoint(ox + int(nx * sw), oy + int(ny * sh))
            for nx, ny in self._norm_poly
        ]
        self.update()

    def _sync_norm_poly(self):
        """Convert current widget-coord poly points to normalised coords."""
        if not self._orig_size or not self._poly_pts_w:
            return
        orig_w, orig_h = self._orig_size
        lw, lh = self.width(), self.height()
        scale = min(lw / orig_w, lh / orig_h)
        ...
        ox, oy = (lw - sw) // 2, (lh - sh) // 2
        self._norm_poly = [
            [(pt.x() - ox) / sw, (pt.y() - oy) / sh]
            for pt in self._poly_pts_w
        ]
    ...
    def mousePressEvent(self, event):
        if self._poly_mode:
            if event.button() == Qt.LeftButton:
                self._poly_pts_w.append(event.pos())
                self._sync_norm_poly()
                self.update()
            elif event.button() == Qt.RightButton:
                if len(self._poly_pts_w) >= 3:
                    self._poly_mode = False
                    self._sync_norm_poly()
                else:
                    self._poly_pts_w = []
    ...
    def mouseReleaseEvent(self, event):
        if self._selecting and event.button() == Qt.LeftButton:
            ...
            self._norm_box = self.get_norm_box()
            self._norm_poly = None
            self._poly_pts_w = []
            self._rb.hide()
            self.update()
    ...
    def resizeEvent(self, event):
        ...
        if self._norm_box and self._orig_size:
            self._recompute_box()
        if self._norm_poly and self._orig_size:
            self._recompute_poly()
    ...
    def paintEvent(self, event):
        ...
        pts = self._poly_pts_w
        if self.show_box and len(pts) >= 1:
            p.setPen(QPen(QColor(255, 220, 0), 2, Qt.SolidLine))
            for i in range(len(pts) - 1):
                p.drawLine(pts[i], pts[i + 1])
            if not self._poly_mode and len(pts) >= 3:
                p.drawLine(pts[-1], pts[0])   # close polygon
            # vertex dots
...
class ViewPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # ── Culvert detection row 1: selection + manual acquire ───────────────
        id_row1 = QHBoxLayout()
        id_row1.addWidget(QLabel("Culvert Detection"))
        self.select_box_btn = QPushButton("Select Box")
        self.select_box_btn.clicked.connect(self._select_box)
        id_row1.addWidget(self.select_box_btn)
        self.select_poly_btn = QPushButton("Select Poly")
        self.select_poly_btn.clicked.connect(self._select_poly)
        id_row1.addWidget(self.select_poly_btn)
        self.show_box_chk = QCheckBox("Show Selected Area")
        self.show_box_chk.setChecked(True)
        self.show_box_chk.toggled.connect(self._toggle_show_box)
        id_row1.addWidget(self.show_box_chk)
        self.acquire_btn = QPushButton("Acquire")
        self.acquire_btn.clicked.connect(self._acquire)
        id_row1.addWidget(self.acquire_btn)
        self.remove_culvert_btn = QPushButton("Remove")
        self.remove_culvert_btn.clicked.connect(self._remove_culvert)
        id_row1.addWidget(self.remove_culvert_btn)
        self.remove_all_btn = QPushButton("Remove All")
        self.remove_all_btn.clicked.connect(self._remove_all_culverts)
        id_row1.addWidget(self.remove_all_btn)
        id_row1.addStretch()
        image_layout.addLayout(id_row1)

        # ── Culvert detection row 2: auto-detect + marker style + navigation ─
        id_row2 = QHBoxLayout()
        self.detect_btn = QPushButton("Detect")
        self.detect_btn.clicked.connect(self._detect_culverts)
        id_row2.addWidget(self.detect_btn)
        self.brightness_spin = QSpinBox()
        self.brightness_spin.setRange(100, 254)
        self.brightness_spin.setValue(190)
        self.brightness_spin.setPrefix("bright ")
        self.brightness_spin.setFixedWidth(165)
        id_row2.addWidget(self.brightness_spin)
        self.id_shape_combo = QComboBox()
        self.id_shape_combo.addItems(["Circle", "Square", "Triangle"])
        id_row2.addWidget(self.id_shape_combo)
        self.id_color_combo = QComboBox()
        self.id_color_combo.addItems(["red", "blue", "green", "orange", "purple", "yellow"])
        id_row2.addWidget(self.id_color_combo)
        self.id_size_combo = QComboBox()
        self.id_size_combo.addItems(["Small", "Medium", "Large"])
        self.id_size_combo.setCurrentText("Medium")
        id_row2.addWidget(self.id_size_combo)
        self.prev_det_btn = QPushButton("← Prev Det")
        self.prev_det_btn.clicked.connect(self._prev_detected)
        id_row2.addWidget(self.prev_det_btn)
        self.next_det_btn = QPushButton("Next Det →")
        self.next_det_btn.clicked.connect(self._next_detected)
        id_row2.addWidget(self.next_det_btn)
        id_row2.addStretch()
        image_layout.addLayout(id_row2)

        self.splitter.addWidget(image_widget)
        self._detect_worker = None   # QTimer when detection is running
    ...
    def _load_culvert_settings(self):
        ...
        if 'poly' in s and s['poly']:
            self.image_label.set_norm_poly(s['poly'])
        elif 'box' in s:
            self.image_label.set_norm_box(s['box'])

    def _save_culvert_settings(self):
        norm_poly = self.image_label.get_norm_poly()
        s = {
            ...
            'poly':  norm_poly if norm_poly and len(norm_poly) >= 3 else None,
        }
    ...
    def _remove_all_culverts(self):
        ...
        reply = QMessageBox.question(self, "Remove All",
            "Remove ALL acquired culverts for this project?",
        ...
        if os.path.exists(csv_path):
            ...
            os.remove(csv_path)
        self.web_view.page().runJavaScript(
            "if(window._culvertMarkers){"
            "  Object.values(window._culvertMarkers).forEach(function(m){m.remove();});"
            "  window._culvertMarkers={};"
            "}"
        )
        self.coords_label.setText("All culverts removed.")

    def _select_box(self):
        self.image_label.enable_selection()

    def _select_poly(self):
        self.image_label.enable_poly_selection()

    def _detected_indices(self):
        """Return sorted list of points_with_files indices that have a culvert detection."""
        ...
        with open(csv_path, newline='') as f:
            for row in csv.DictReader(f):
                detected_frames.add(row.get('source_frame', ''))
        indices = [i for i, (_, _, fname) in enumerate(self.communicator.points_with_files)
                   if fname in detected_frames]
        return sorted(indices)

    def _prev_detected(self):
        indices = self._detected_indices()
        if not indices:
            self.coords_label.setText("No detected culverts.")
            return
    ...
    def _detect_culverts(self):
        if not self.communicator or not self._project_name:
            QMessageBox.warning(self, "No Project", "Load a project first.")
            return
        norm_poly = self.image_label.get_norm_poly()
        norm_box  = self.image_label.get_norm_box()
        if norm_poly and len(norm_poly) >= 3:
            use_poly = True
        elif norm_box and norm_box['w'] > 0 and norm_box['h'] > 0:
            use_poly = False
        else:
            QMessageBox.warning(self, "No Selection", "Draw a box or polygon area first.")
            return
        csv_path     = project_culverts_csv(self._project_name)
        already = set()
        if os.path.exists(csv_path):
        ...
        # Build work queue — skip already-acquired frames
        queue = [(lat, lon, fname)
                 for lat, lon, fname in self.communicator.points_with_files
                 if fname not in already]

        if not queue:
            self.coords_label.setText("All frames already checked.")
            return

        self._det_total      = len(queue)
        self._det_found      = 0
        self._det_new_rows   = []
        self._det_norm_box   = norm_box if not use_poly else None
        self._det_norm_poly  = norm_poly if use_poly else None
        self._det_brightness = self.brightness_spin.value()
        self._det_frames_dir = project_frames_dir(self._project_name)
        self._det_culverts_dir = os.path.join(PROJECTS_DIR, self._project_name, "culverts")
        self._det_csv_path   = csv_path
        os.makedirs(self._det_culverts_dir, exist_ok=True)

        self.detect_btn.setEnabled(False)
        self.acquire_btn.setEnabled(False)
        self.coords_label.setText(f"Detecting... 0 / {self._det_total}")

        # Submit all frames to process pool
        workers = min(os.cpu_count() or 4, 8)
        self._det_executor = ProcessPoolExecutor(max_workers=workers)
        norm_box   = self._det_norm_box
        norm_poly  = self._det_norm_poly
        brightness = self._det_brightness
        frames_dir = self._det_frames_dir
        culverts_dir = self._det_culverts_dir
        self._det_futures = {}
        for lat, lon, fname in queue:
            fpath = os.path.join(frames_dir, fname)
            job = (fpath, norm_box, norm_poly, brightness, culverts_dir)
            fut = self._det_executor.submit(_detect_frame_job, job)
            self._det_futures[fut] = (lat, lon, fname)
        self._det_done_count = 0

        self._detect_worker = QTimer(self)
        self._detect_worker.setInterval(100)
        self._detect_worker.timeout.connect(self._poll_detect_futures)
        self._detect_worker.start()

    def _poll_detect_futures(self):
        """Called every 100 ms; harvest completed futures and update progress."""
        completed_this_tick = []
        for fut, meta in list(self._det_futures.items()):
            if fut.done():
                completed_this_tick.append((fut, meta))

        for fut, (lat, lon, fname) in completed_this_tick:
            del self._det_futures[fut]
            self._det_done_count += 1
            try:
                result = fut.result()
            except Exception:
                result = None
            if result:
                self._det_new_rows.append({
                    'name': 'culvert', 'latitude': lat, 'longitude': lon,
                    'source_frame': fname, 'saved_image': result['img_fn'],
                    'box_x': result['bx'], 'box_y': result['by'],
                    'box_w': result['bw'], 'box_h': result['bh'],
                })
                self._det_found += 1

        self.coords_label.setText(
            f"Detecting... {self._det_done_count} / {self._det_total}  found: {self._det_found}"
        )

        if not self._det_futures:
            self._detect_worker.stop()
            self._det_executor.shutdown(wait=False)
            self._on_detect_finished()

    def _on_detect_finished(self):
        self.detect_btn.setEnabled(True)
        self.acquire_btn.setEnabled(True)
        self.coords_label.setText(
            f"Detection complete — {self._det_found} new culvert(s) found from {self._det_total} frames."
        )
        if not self._det_new_rows:
            return
        write_header = not os.path.exists(self._det_csv_path)
        with open(self._det_csv_path, 'a', newline='') as f:
            w = csv.DictWriter(f, fieldnames=[
                'name','latitude','longitude','source_frame',
                'saved_image','box_x','box_y','box_w','box_h'])
            if write_header:
                w.writeheader()
            w.writerows(self._det_new_rows)
        ...
        for d in self._det_new_rows:
            fname = d['source_frame']
            lat, lon = d['latitude'], d['longitude']
            safe  = fname.replace("'", "\\'")
            popup = f"'culvert (det)<br>{lat:.5f}, {lon:.5f}'"
            js = (f"window._culvertMarkers = window._culvertMarkers || {{}};"
                  f"window._culvertMarkers['{safe}'] = "
    ...
    def _acquire(self):
        ...
        if norm_poly and len(norm_poly) >= 3:
            import numpy as np
            pts = np.array([[int(nx * ow), int(ny * oh)] for nx, ny in norm_poly], dtype=np.int32)
            px, py, pw, ph = cv2.boundingRect(pts)
            crop_rect = QRect(px, py, pw, ph)
        else:
            crop_rect = self.image_label.get_crop_rect(ow, oh)
        if not (crop_rect and crop_rect.isValid() and crop_rect.width() > 0 and crop_rect.height() > 0):
            QMessageBox.warning(self, "No Selection", "Draw a box or polygon area first.")
            return
