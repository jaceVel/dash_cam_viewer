#!/usr/bin/env python3
"""
Dash Cam Viewer v7 - Combined Frame Extractor and Map Viewer
Culvert detection using OpenCV whitish vertical rectangle detection in box area.
Box selection required for acquire. No YOLO.
"""
...
class ViewPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        id_row.addWidget(self.select_poly_btn)
        ...
        id_row.addWidget(self.remove_all_btn)
        ...
        id_row.addWidget(self.brightness_spin)
    ...
    def _detect_culverts(self):
        ...
        self._detect_worker.setInterval(0)
        self._detect_worker.timeout.connect(self._detect_next_frame)
    ...
    def _detect_next_frame(self):
        ...
        lat, lon, fname = self._det_queue.pop(0)
        done = self._det_total - len(self._det_queue)
        fpath = os.path.join(self._det_frames_dir, fname)
        if not os.path.exists(fpath):
            self.coords_label.setText(f"Detecting... {done} / {self._det_total}  found: {self._det_found}")
            return
        ...
        if self._det_norm_poly:
            import numpy as np
            pts = np.array([[int(nx * iw), int(ny * ih)] for nx, ny in self._det_norm_poly], dtype=np.int32)
            ...
            nx = self._det_norm_box['x']
            ny = self._det_norm_box['y']
            nw = self._det_norm_box['w']
            nh = self._det_norm_box['h']
        ...
        mask = cv2.inRange(hsv, (0, 0, self._det_brightness), (180, 80, 255))
        ...
        # Find a valid pair: horizontal separation + vertical overlap
        valid_pair = None
        for i in range(len(rects)):
            for j in range(i + 1, len(rects)):
                ...
                # Horizontal separation: gap between right edge of a and left edge of b
                h_gap = bx2 - (ax + aw)
                if h_gap < 3:          # must have at least 3px gap
                    continue
                if h_gap > bw * 0.5:   # gap must not be more than half the box width
                    continue
                # Vertical overlap: Y ranges must overlap by at least 40% of shorter height
        ...
        for rx, ry, rw, rh in valid_pair:
            ...
            cv2.putText(annotated, f"{rw}x{rh} b{self._det_brightness}", (rx, ry - 4),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1)
        cv2.imwrite(os.path.join(self._det_culverts_dir, img_fn), annotated,
