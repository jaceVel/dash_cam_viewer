#!/usr/bin/env python3
"""
Dash Cam Viewer v6 - Combined Frame Extractor and Map Viewer
Adds YOLO culvert detection with iterative train/detect/validate loop.
"""
...
CULVERT_DATASET_DIR    = os.path.join(PROJECTS_DIR, "culvert_dataset")
CULVERT_MODEL_DIR      = os.path.join(PROJECTS_DIR, "culvert_model")
CULVERT_MODEL_PATH     = os.path.join(CULVERT_MODEL_DIR, "weights", "best.pt")
FPS_FALLBACK = 30.0
...
_TRAIN_SCRIPT = r"""
import sys, os, csv, shutil, struct, re

project_name, csv_path, frames_dir, dataset_dir, model_dir, base_model, epochs = \
...
if not os.path.exists(csv_path):
    print("ERROR: No culverts.csv found", flush=True)
    sys.exit(1)
...
for r in rows:
    try:
        valid.append({'frame': r['source_frame'],
                      'bx': int(r['box_x']), 'by': int(r['box_y']),
                      'bw': int(r['box_w']), 'bh': int(r['box_h'])})
    except (KeyError, ValueError): pass

if not valid:
    print("ERROR: No valid bounding boxes in culverts.csv", flush=True)
    sys.exit(1)

print(f"Positive samples: {len(valid)}", flush=True)
...
positive_set = {v['frame'] for v in valid}

neg_set = set()
for fname in positive_set:
    ...
    for offset in (-2, 2):
        ...
        if 0 <= ni < len(all_frames):
            c = all_frames[ni]
            if c not in positive_set: neg_set.add(c)

print(f"Negative samples: {len(neg_set)}", flush=True)
...
grouped = {}
for v in valid: grouped.setdefault(v['frame'], []).append(v)
...
for fname, boxes in grouped.items():
    ...
    shutil.copy2(src, os.path.join(img_train, fname))
...
with open(yaml_path, 'w') as yf:
    ...
    yf.write("nc: 1\n")
    yf.write("names: ['culvert']\n")

print(f"Dataset ready: {len(grouped)} positive, {len(neg_set)} negative", flush=True)
...
frames_dir, model_path, conf, culverts_dir, csv_path, params_file = \
...
for fname in frames:
    ...
    for r in results:
        ...
        for box in r.boxes:
            ...
            img_fn = f"culvert_yolo_{ts}.jpg"
            crop = img[by:by+bh, bx:bx+bw]
            cv2.imwrite(os.path.join(culverts_dir, img_fn), crop,
                        [int(cv2.IMWRITE_JPEG_QUALITY), 90])
            new_rows.append({"name":"culvert","latitude":lat,"longitude":lon,
                              "source_frame":fname,"saved_image":img_fn,
                              "box_x":bx,"box_y":by,"box_w":bw,"box_h":bh})
            print(f"Found culvert in {fname}", flush=True)
...
class YoloTrainWorker(QThread):
    ...
    def run(self):
        try:
            ...
            csv_path   = project_culverts_csv(self.project_name)
            frames_dir = project_frames_dir(self.project_name)
            base       = CULVERT_MODEL_PATH if os.path.exists(CULVERT_MODEL_PATH) else 'yolov8n.pt'
            epochs     = 50
            self.log.emit(f"Launching training subprocess (base: {os.path.basename(base)}, epochs: {epochs})...")
            proc = subprocess.Popen(
                [sys.executable, '-c', _TRAIN_SCRIPT,
                 self.project_name, csv_path, frames_dir,
                 CULVERT_DATASET_DIR, CULVERT_MODEL_DIR, base, str(epochs)],
            ...
            if proc.returncode == 0:
                self.log.emit(f"Training complete. Model: {CULVERT_MODEL_PATH}")
                self.finished.emit(True)
            else:
...
class YoloDetectWorker(QThread):
    ...
    def run(self):
        if not os.path.exists(CULVERT_MODEL_PATH):
        ...
        frames_dir   = project_frames_dir(self.project_name)
        csv_path     = project_culverts_csv(self.project_name)
        culverts_dir = os.path.join(PROJECTS_DIR, self.project_name, "culverts")
        ...
        proc = subprocess.Popen(
            [sys.executable, '-c', _DETECT_SCRIPT,
             frames_dir, CULVERT_MODEL_PATH, str(self.confidence),
             culverts_dir, csv_path, params_file],
            stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
            text=True, bufsize=1, encoding='utf-8', errors='replace',
        )
        for line in proc.stdout:
...
class ViewPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        self.conf_spin.setFixedWidth(135)
    ...
    def _on_train_finished(self, success):
        ...
        if success:
            self.coords_label.setText(
                f"Training complete — model ready at culvert_model/weights/best.pt"
            )
    ...
    def _on_detect_finished(self, success):
        ...
        for d in rows:
            ...
            safe  = fname.replace("'", "\\'")
            popup = f"'culvert (YOLO)<br>{lat:.5f}, {lon:.5f}'"
